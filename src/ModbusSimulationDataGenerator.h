...
class ModbusSimulationDataGenerator
{
  ...
  protected: // Modbus specific
    ...
    void SendReadFileRecordRequest( U8 DeviceID, U8 ByteCount, U8 SubReqReferenceTypes[], U16 SubReqFileNumbers[],
                                    U16 SubReqRecordNumbers[], U16 SubReqRecordLengths[] );

    // this function generates the request for the Write File Record (0x15) command (note the forced size of the 2D array, no reason to
    // implement points for this, overkill for simulation functions
    void SendWriteFileRecordRequest( U8 DeviceID, U8 ByteCount, U8 SubReqReferenceTypes[], U16 SubReqFileNumbers[],
                                     U16 SubReqRecordNumbers[], U16 SubReqRecordLengths[], U16 SubReqRecordData[ 1 ][ 3 ] );

    // this function generates the request for the Read/Write Multiple Registers (0x17) command
    void SendReadWriteMultipleRegisters( U8 DeviceID, U16 ReadStartingAddress, U16 QuantityToRead, U16 WriteStartingAddress,
                                         U16 QuantityToWrite, U8 WriteByteCount, U16 Values[] );
